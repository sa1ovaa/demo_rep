...
def draw_ui(current_tool, current_color, current_size):
    ...
    status_text = f"Tool: {current_tool.upper()} | Color: {current_color} | Size: {current_size}px"
    controls_text = "T Text | F Fill | S Save | 1-3 Size"

    screen.blit(ui_font.render(status_text, True, COLORS['black']), (10, 5))
    screen.blit(ui_font.render(controls_text, True, COLORS['black']),
                (WIDTH - 250, 5))
...
def main():
    ...
    while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if typing_mode:
                    if event.key == pygame.K_RETURN:
                        canvas.blit(font.render(text_input, True, current_color_rgb), text_pos)
                        typing_mode = False
                        text_input = ""
                ...
                if event.key == pygame.K_s and (pygame.key.get_mods() & pygame.KMOD_CTRL):
                    name = f"canvas_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        ...
        if drawing:
            ...
            if current_tool == 'line':
                pygame.draw.line(screen, current_color_rgb, start_pos, pos, current_size)
            elif current_tool not in ['pencil', 'eraser', 'fill', 'text']:
                draw_shape(screen, current_color_rgb, start_pos, pos, current_tool, current_size)

        if typing_mode:
            txt = font.render(text_input + "|", True, current_color_rgb)
