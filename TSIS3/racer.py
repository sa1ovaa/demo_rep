...
def draw_hud(screen, fontt, collected, distance, active_pu, pu_end, hp):
    screen.blit(fontt.render(f"Score:{collected}  Dist:{int(distance)}m", True, "black"), (5, 5))
    screen.blit(fontt.render(f"HP:{hp}", True, "red"), (5, 45))
    if active_pu:
...
def play_game(screen, username):
    ...
    while running:
        ...
        if reset_nitro and active_pu != "nitro":
            player.speed = base_speed
            reset_nitro = False
        ...
        draw_hud(screen, fontt, collected, distance, active_pu, pu_end, player.hp)
